...
async function recomputeChartIfBirthChanged(
  userId: ObjectId,
  existing: { dob?: string | null; birthTime?: string | null; birthPlace?: string | null } | null,
  incoming: { dob?: string | null; birthTime?: string | null; birthPlace?: string | null }
) {
  ...
  const birthChanged =
    ...
    (incoming.birthTime !== undefined && incoming.birthTime !== existing?.birthTime) ||
    (incoming.birthPlace !== undefined && incoming.birthPlace !== existing?.birthPlace);
...
export async function PUT(request: Request) {
  try {
    ...
    void recomputeChartIfBirthChanged(
      userId,
      before ? { dob: before.dob, birthTime: before.birthTime, birthPlace: before.birthPlace } : null,
