import { buildChartDigest, relevantChartFor } from "@/lib/astrology/chart";
...
- yogasAndDoshas: ALL yogas and doshas relevant to the topic MUST be included — these are the primary analytical framework. Explain the mechanism (which grahas, which bhavas, what it produces). Include beneficial yogas AND harmful doshas — do not omit either. Where classicalTextGrounding provides a citation relevant to a yoga/dosha/placement, reference it directly rather than asserting the rule as if from nowhere — never invent a citation that isn't in classicalTextGrounding.
...
export async function runReasoningPass(
  ...
  const chartContext = {
    ...
    allYogas: chart.yogas.map((y) => ({
      name: y.name,
      type: y.type,
      strength: y.strength,
      description: y.description,
    })),
    allDoshas: chart.doshas
      .filter((d) => d.severity !== "cancelled" && d.severity !== "none")
      .map((d) => ({
        name: d.name,
        severity: d.severity,
    ...
    classicalTextGrounding: buildChartDigest(chart, transits)
      .classicalGrounding,
  };
