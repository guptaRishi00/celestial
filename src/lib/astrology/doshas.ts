...
export function detectDoshas(
  ...
  if (rahu && ketu) {
    ...
    const forwardCount = otherPlanets.filter((p) =>
      inForwardArc(p.longitude),
    ).length;
    const allOneSide =
      forwardCount === otherPlanets.length || forwardCount === 0;
    if (allOneSide) {
      out.push({
        ...
        severity: "high",
        description:
          "All seven major planets are hemmed between the Rahu-Ketu axis — indicates karmic obstacles, delays in achievements, and a sense of struggle to manifest one's goals. Often manifests as obstacles before breakthroughs.",
      });
    }
  }

  // Vish Yoga — Saturn and Moon conjunct in the same sign (the actual classical name
  // for this combination). Previously also flagged on a plain 7th-house opposition,
  // which isn't a distinctly named dosha — every planet has a 7th aspect on its
  // opposite house, so that case has been dropped rather than mislabeled as one.
  if (saturn && moon && saturn.sign === moon.sign) {
    out.push({
      name: "Vish Yoga",
      present: true,
      severity: "moderate",
      description:
        "Saturn and Moon conjunct in the same sign — the classical Vish Yoga. In its harsher expression: emotional heaviness, melancholy, and mental strain. Well-supported (e.g. by benefic aspects or a strong sign), it instead tends to give depth, discipline, and real aptitude for research, healing, or occult study — not a purely negative placement.",
    });
  }
  ...
  if (jupiter && rahu && jupiter.sign === rahu.sign) {
    out.push({
      ...
      severity: "moderate",
      description:
        "Jupiter conjunct Rahu — wisdom mixed with unconventional or unorthodox influences. Can give sharp intellect and innovative thinking but also tendency toward questionable advice or moral confusion. Spiritual study often resolves it.",
    });
  }
  if (jupiter && ketu && jupiter.sign === ketu.sign) {
    out.push({
      ...
      severity: "mild",
      description:
        "Jupiter conjunct Ketu — gives strong spiritual inclinations, detachment from materialism, but can scatter focus from worldly responsibilities. Often produces deep philosophers or seekers.",
    });
  }
